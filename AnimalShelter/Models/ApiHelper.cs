...
namespace AnimalShelterClient.Models
{
  class ApiHelper
  {
    public static async Task<string> GetAll(string species, string sex, string color, string breed)
    {
      RestClient client = new RestClient("http://localhost:5004/api");
      RestRequest request = new RestRequest($"animals?species={species}&sex={sex}&color={color}&breed={breed}", Method.GET);
